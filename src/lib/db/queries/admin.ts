...
export async function getAdminRevenue() {
  ...
  const rows = await Promise.all(periods.map((start) => db.select({ charges: sql<number>`COALESCE(SUM(${usageRecords.cost}), 0)`, margin: sql<number>`COALESCE(SUM(${usageRecords.cost} - COALESCE(${usageRecords.actualCost}, CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END, 0)), 0)` }).from(usageRecords).where(gte(usageRecords.createdAt, start))));
  ...
  const [allTime = { charges: 0, margin: 0 }] = await Promise.all([db.select({ charges: sql<number>`COALESCE(SUM(${usageRecords.cost}), 0)`, margin: sql<number>`COALESCE(SUM(${usageRecords.cost} - COALESCE(${usageRecords.actualCost}, CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END, 0)), 0)` }).from(usageRecords)]).then((r) => r.map((row) => ({ charges: Number(row[0]?.charges ?? 0), margin: Number(row[0]?.margin ?? 0) })));
  const modelRows = await db.select({ model: usageRecords.model, requests: sql<number>`COUNT(*)`, userCost: sql<number>`COALESCE(SUM(${usageRecords.cost}), 0)`, upstreamCost: sql<number>`COALESCE(SUM(COALESCE(${usageRecords.actualCost}, CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END, 0)), 0)`, margin: sql<number>`COALESCE(SUM(${usageRecords.cost} - COALESCE(${usageRecords.actualCost}, CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END, 0)), 0)` }).from(usageRecords).groupBy(usageRecords.model).orderBy(sql`COALESCE(SUM(${usageRecords.cost} - COALESCE(${usageRecords.actualCost}, CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END, 0)), 0) DESC`);
