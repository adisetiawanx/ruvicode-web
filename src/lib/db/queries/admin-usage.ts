...
export async function getAdminUsage() {
  ...
  const rows = await db.select({ id: usageRecords.id, model: usageRecords.model, keyLabel: apiKeys.label, promptTokens: usageRecords.promptTokens, completionTokens: usageRecords.completionTokens, reasoningTokens: usageRecords.reasoningTokens, cacheReadTokens: usageRecords.cacheReadTokens, cost: usageRecords.cost, upstreamCost: usageRecords.upstreamCost, status: usageRecords.status, requestId: usageRecords.requestId, createdAt: usageRecords.createdAt }).from(usageRecords).leftJoin(apiKeys, eq(apiKeys.id, usageRecords.apiKeyId)).orderBy(desc(usageRecords.createdAt)).limit(100);
