...
// Provider cost per row prefers market_cost (the estimated real wallet
// charge) and falls back to upstream_cost only for legacy rows written
// before market_cost existed.
const providerCostExpr = sql<number>`COALESCE(CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END, 0)`;

export async function getAdminUsage() {
  if (!isDbAvailable()) return { rows: [], models: [], keys: [], volume: [] };
  const rows = await db.select({ id: usageRecords.id, model: usageRecords.model, userEmail: user.email, keyLabel: apiKeys.label, promptTokens: usageRecords.promptTokens, completionTokens: usageRecords.completionTokens, reasoningTokens: usageRecords.reasoningTokens, cacheReadTokens: usageRecords.cacheReadTokens, cost: usageRecords.cost, providerCost: providerCostExpr, status: usageRecords.status, requestId: usageRecords.requestId, createdAt: usageRecords.createdAt }).from(usageRecords).leftJoin(apiKeys, eq(apiKeys.id, usageRecords.apiKeyId)).leftJoin(user, eq(user.id, usageRecords.userId)).orderBy(desc(usageRecords.createdAt)).limit(100);
  const models = await db.select({ model: usageRecords.model, requests: sql<number>`COUNT(*)`, cost: sql<number>`COALESCE(SUM(${usageRecords.cost}), 0)`, providerCost: sql<number>`COALESCE(SUM(CASE WHEN ${usageRecords.marketCost} > 0 THEN ${usageRecords.marketCost} ELSE ${usageRecords.upstreamCost} END), 0)` }).from(usageRecords).groupBy(usageRecords.model).orderBy(sql`COUNT(*) DESC`);
