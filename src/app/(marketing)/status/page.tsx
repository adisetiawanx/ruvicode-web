            ...
            <div className="overflow-hidden rounded-xl border border-border-default">
              <div className="grid grid-cols-1 divide-y divide-border-subtle md:grid-cols-2 md:divide-x">
                {models.map((m, i) => (
                  <div
                    key={m.model}
                    className={`flex items-center justify-between px-4 py-2.5 ${i % 2 === 1 ? "md:border-l-0" : ""} ${i < models.length - (models.length % 2 === 0 ? 2 : 1) ? "border-b border-border-subtle md:border-b" : ""}`}
                  >
